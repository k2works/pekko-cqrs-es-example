 ...
 *
 * Pekkoアクターを使用して在庫管理のビジネスロジックを実行する 楽観的ロック制御により並行アクセス時の整合性を保証
 */
private[inventory] final class InventoryUseCaseImpl(
  ...
  override def createInventory(
    ...
    for {
      _ <- ZIO.succeed(
        logger.info(
          s"Creating Inventory for product: ${productId.asString}, zone: ${warehouseZoneId.asString}")
      )
  ...
  override def receiveInventory(
    ...
    for {
      _ <- ZIO.succeed(
        logger.info(
          s"Receiving inventory ${inventoryId.asString}, quantity: ${quantity.amount}, version: ${expectedVersion.value}")
      )
      ...
      result <- reply match {
        case InventoryProtocol.ReceiveInventorySucceeded(id, newVersion) =>
          ZIO.succeed(logger.info(
            s"Inventory receive succeeded for ID: ${id.asString}, new version: ${newVersion.value}")) *>
        ...
        case InventoryProtocol.ReceiveInventoryFailed(id, reason) =>
          reason match {
            case io.github.j5ik2o.pcqrses.command.domain.inventory.ReceiveInventoryError.VersionMismatch =>
              ZIO.fail(
                InventoryUseCaseError.VersionMismatch(s"Version mismatch for ID ${id.asString}"))
            case _ =>
              ZIO.fail(
                InventoryUseCaseError.ReceiveFailed(
                  s"Receive failed for ID ${id.asString}: $reason"))
          }
      }
    } yield result
  ...
  override def reserveInventory(
    ...
    for {
      _ <- ZIO.succeed(
        logger.info(
          s"Reserving inventory ${inventoryId.asString}, quantity: ${quantity.amount}, version: ${expectedVersion.value}")
      )
      ...
      result <- reply match {
        case InventoryProtocol.ReserveInventorySucceeded(id, newVersion) =>
          ZIO.succeed(logger.info(
            s"Inventory reserve succeeded for ID: ${id.asString}, new version: ${newVersion.value}")) *>
            ZIO.succeed(newVersion)
        case InventoryProtocol.ReserveInventoryFailed(id, reason) =>
          reason match {
            ...
            case io.github.j5ik2o.pcqrses.command.domain.inventory.ReserveInventoryError.InsufficientStock =>
              ZIO.fail(
                InventoryUseCaseError.ReserveFailed(s"Insufficient stock for ID ${id.asString}"))
          }
      }
    } yield result
  ...
  override def releaseInventory(
    ...
    for {
      _ <- ZIO.succeed(
        logger.info(
          s"Releasing inventory ${inventoryId.asString}, quantity: ${quantity.amount}, version: ${expectedVersion.value}")
      )
      ...
      result <- reply match {
        case InventoryProtocol.ReleaseInventorySucceeded(id, newVersion) =>
          ZIO.succeed(logger.info(
            s"Inventory release succeeded for ID: ${id.asString}, new version: ${newVersion.value}")) *>
            ZIO.succeed(newVersion)
        case InventoryProtocol.ReleaseInventoryFailed(id, reason) =>
          reason match {
            ...
            case io.github.j5ik2o.pcqrses.command.domain.inventory.ReleaseInventoryError.InsufficientReserved =>
              ZIO.fail(
                InventoryUseCaseError.ReleaseFailed(s"Insufficient reserved for ID ${id.asString}"))
          }
      }
    } yield result
  ...
  override def issueInventory(
    ...
    for {
      _ <- ZIO.succeed(
        logger.info(
          s"Issuing inventory ${inventoryId.asString}, quantity: ${quantity.amount}, version: ${expectedVersion.value}")
      )
      ...
      result <- reply match {
        case InventoryProtocol.IssueInventorySucceeded(id, newVersion) =>
          ZIO.succeed(logger.info(
            s"Inventory issue succeeded for ID: ${id.asString}, new version: ${newVersion.value}")) *>
            ZIO.succeed(newVersion)
        case InventoryProtocol.IssueInventoryFailed(id, reason) =>
          reason match {
            ...
            case io.github.j5ik2o.pcqrses.command.domain.inventory.IssueInventoryError.InsufficientReserved =>
              ZIO.fail(
                InventoryUseCaseError.IssueFailed(s"Insufficient reserved for ID ${id.asString}"))
          }
      }
    } yield result
  ...
  override def adjustInventory(
    ...
    for {
      _ <- ZIO.succeed(
        logger.info(
          s"Adjusting inventory ${inventoryId.asString}, new quantity: ${newQuantity.amount}, reason: $reason, version: ${expectedVersion.value}")
      )
      ...
      result <- reply match {
        case InventoryProtocol.AdjustInventorySucceeded(id, newVersion) =>
          ZIO.succeed(logger.info(
            s"Inventory adjust succeeded for ID: ${id.asString}, new version: ${newVersion.value}")) *>
            ZIO.succeed(newVersion)
        case InventoryProtocol.AdjustInventoryFailed(id, reason) =>
          reason match {
            ...
            case io.github.j5ik2o.pcqrses.command.domain.inventory.AdjustInventoryError.ResultingNegativeQuantity =>
              ZIO.fail(
                InventoryUseCaseError.AdjustFailed(
                  s"Adjustment would result in negative quantity for ID ${id.asString}"))
          }
      }
    } yield result
