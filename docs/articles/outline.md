...
2. **Customer（顧客）**: 顧客情報、与信情報、支払条件
3. **Supplier（仕入先）**: 仕入先情報、支払条件、評価
4. **Warehouse（倉庫）**: 倉庫情報、ロケーション
5. **AccountSubject（勘定科目）**: 勘定科目体系
6. **Organization（組織）**: 組織構造
7. **Employee（従業員）**: 従業員情報、権限
8. **CodeMaster（コードマスター）**: 共通コードと値
...
package "発注管理サービス (Part 5)" {
  [発注管理] --> [商品マスター] : 商品・仕入先参照
  [発注管理] --> [仕入先マスター] : 仕入先情報参照
}

package "会計サービス (Part 6)" {
  [会計処理] --> [顧客マスター] : 売掛金補助科目
  [会計処理] --> [仕入先マスター] : 買掛金補助科目
  [会計処理] --> [勘定科目マスター] : 勘定科目体系
}
@enduml
```
...
- **顧客マスター**: 430社の顧客、与信管理、支払条件
- **仕入先マスター**: 200社の仕入先、評価、支払条件
- **勘定科目マスター**: 500科目の勘定科目体系
